...
public class ContestRepository implements IContestRepository {
    ...
    public Contest save(Contest entity) {
        if (entity.getId() == null) {
            autoIncrement++;
            Contest c = new Contest(Integer.toString(autoIncrement), entity.getName(), entity.getQuestions(),
                    entity.getLevel(), entity.getCreator(), entity.getContestStatus());
    ...
    public List<Contest> findAllContestLevelWise(Level level) {
        // List<Contest> allContestLevelWise = new ArrayList<Contest>();
        // for(Contest contest:contestMap.values()){
        // if(contest.getLevel() == level){
        // allContestLevelWise.add(contest);
        // }
        // }
        return contestMap.values().stream().filter(contest -> contest.getLevel().equals(level))
