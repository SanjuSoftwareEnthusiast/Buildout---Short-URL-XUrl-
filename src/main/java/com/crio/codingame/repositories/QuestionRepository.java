...
public class QuestionRepository implements IQuestionRepository {
    ...
    public Question save(Question entity) {
        if (entity.getId() == null) {
            autoIncrement++;
            Question q = new Question(Integer.toString(autoIncrement), entity.getTitle(), entity.getLevel(),
                    entity.getScore());
    ...
    public List<Question> findAllQuestionLevelWise(Level level) {
        List<Question> allQuestionLevelWise = new ArrayList<Question>();
        for (Question question : questionMap.values()) {
            if (question.getLevel() == level) {
                allQuestionLevelWise.add(question);
            }
        }
        return allQuestionLevelWise;
    }
